...
function ModuleSidebar({ modules, onFeedbackClick }) {
  const [expandedModules, setExpandedModules] = useState(new Set());
      ...
      <div className="p-2">
        {modules.map((module) => (
          <div key={module.id} className="mb-2">
            <button
              onClick={() => toggleModule(module.id)}
              className="w-full flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg cursor-pointer"
            ...
            {expandedModules.has(module.id) && (
              <div className="ml-4 mt-1 space-y-1">
                {module.contents.map((content) => (
                  <button
                    key={content.id}
                    onClick={() => content.onSelect ? content.onSelect(content) : null}
                    className="w-full flex items-center p-2 text-sm text-gray-600 hover:bg-blue-50 hover:text-blue-600 rounded-md"
                  >
                    {content.type === 'video' ? (
                      <FiPlay className="mr-2 w-4 h-4" />
        ...
        <button
          ...
          className="w-full flex items-center justify-center p-3 mt-4 bg-blue-500 text-white hover:bg-blue-600 rounded-lg"
        >
          Feedback
        </button>
      </div>
